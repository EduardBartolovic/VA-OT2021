...
\begin{document}
	...
	\[ f(x) = x* \tanh (\ln (1+e^x)) \]
	Die Komponenten in der Architektur wurde auch ersetzt. Im \textit{Backbone} läuft nun \textit{CSPDarknet53}. Im \textit{Neck}  wird \textit{Spatial pyramid pooling} (SPP) und \textit{Path Aggregation Network} (PAN) verwendet.++++++++++\\
	Am \textit{Head} wurde nichts geändert und  ist damit identisch zu YoloV3 \cite{b2}.
	\begin{figure}[!h]
		\begin{center}
