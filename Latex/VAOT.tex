...
\begin{document}
	...
	Eine Schwierigkeit bei der Objekterkennung ist, dass die Bewegungen hauptsächlich bei großen Intensitätsunterschieden wie der Wechsel von Straße zu Fahrzeuganfang und von Fahrzeugende zur Straße, erkannt werden. Große Flächen wie die Motorhaube, Windschutzscheibe und die Dachfläche werden nicht erkannt, da diese so lange einen Pixel mit einem sehr ähnlichen Intensitätswert „bedecken“, dass dieser zwischendurch als Hintergrund festgelegt wird. Dabei kann es passieren, dass je nach Thresholdgröße mehrere Objekte in einem Fahrzeug erkannt werden. Dieses Verhalten zeigt sich in Abbildung \ref{mogFehler1}, \ref{mogFehler2} und \ref{mogFehler3}. Ein weiteres Problem ist es wenn auf der neben den Objekten Kanten liegen. Diese Kanten sorgen zum Beispiel dafür, das der Bus in der Grafik \ref{mogFehler2} und \ref{mogFehler4} wird zweigeteilt. Der Grund hierfür dürfte die Bordsteinkante und der Asphaltflick auf dem Radweg sein. All diese Probleme führen dazu das der Bus der in den Abbildungen \ref{mogFehler2} und \ref{mogFehler3} insgesamt dreimal erkannt wird.
	\begin{figure}[!h]
		\begin{center}
...
\end{figure}
	...
	\begin{figure}[!h]
		...
		\end{center}
	\end{figure}
	Diese Ankerboxen kann man mittels Clusteralgorithmen wie K-Means\cite{kmeans} und den Boundingboxen aus dem Datensatz berechnen. Dies hat vor allem den Vorteil, dass die Boundingboxen besser zu den Objekten des jeweiligen Datensatzes passen.\\
